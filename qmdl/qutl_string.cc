...
QINT qstrint(QINT system, QPNT str, QINT *len)
{
	QUCHR chchar, *pchar;
...
QINT qstrprmcb(QPNT str, QUCHR chr, qstr_prm_cb prm_cb, QINT count, ...)
{
	...
	ppprms = (QPNT *)qprmmakev(NULL, vbuff, sizeof(vbuff), NULL, NULL, marker, &count);
	va_end(marker);
...
QINT qstrmatch(QSTR str, QINT size, QINT *flag, QSTR *read, QINT *len)
{
	QUCHR chchar, chbegin, chend, *pbegin, *pdstchar, *pdstread, *psrcread;
	...
	switch(ndstflag)
	{
		case QSTR_DEC:
			...
			while(pdstchar)
			{
				...
				if(psrcread != NULL)
				{
					if(qstrcmp(ncmpflag, pdstchar, psrcread, nsrclen))
					{
						nend = 1;
						break;
					}
